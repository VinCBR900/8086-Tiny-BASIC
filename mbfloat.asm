...
; v0.4 : Precision improvements - STABLE. All 10 tests passing.
;         flt_div: two-stage 32-bit quotient, 7 sig fig precision.
;         flt_mul: step5 A_byte4*B_hi (product>>8 added to CX); xor ah,ah before mul.
;         flt_add: jnb (unsigned) for biased exponent compare (was jge/signed).
;         flt_print: 6-digit rounding using 7th digit as guide; correct carry propagation.
;         flt_from_int_b: now preserves SI (push/pop added).
;         flt_mul: now preserves SI (push/pop around computation).
...
; KNOWN OPEN ITEMS
; ---------------------------------------------------------------------------
; - flt_div divisor truncated to 16 bits (caps precision ~4.5 digits for divisors
;   with significant lower bytes; adequate for current test suite)
; - DIV/0 prints inline rather than raising error to BASIC
; - FLT_T not safe for inter-routine data shuttling when flt_add in call chain
;   (callers must save/restore FLT_A by other means; see T5 harness for example)
; - flt_mul DI clobbered (acceptable since DI used for BASIC's dest pointer, SI preserved)
;
; ---------------------------------------------------------------------------
; MBF5 FORMAT
; ---------------------------------------------------------------------------
...
; =============================================================================
; FLT_FROM_INT_B  AX (signed int16) -> FLT_B  (preserves FLT_A, preserves SI)
...
; Outputs : AX.  Returns -32768..32767. Saturates at +/-32767 on overflow.
; Clobbers: AX, BX, CX
; =============================================================================
flt_to_int:
        mov  al, [FLT_A+0]
        ...
        cmp  al, 16
        jg   fti_sat            ; exponent > 16 -> overflow (BUGFIX: was 15)
        mov  bl, [FLT_A+1]
        and  bl, 0x80
...
fti_zero:
        ...
        ret
fti_sat:
        ; Saturate: +32767 or -32767. Special case: exactly -32768 is representable.
        test byte [FLT_A+1], 0x80
        jz   fti_sat_pos
        ; Negative: check if exactly -32768 (exp=0x90, mant=0x80,0,0,0)
...
fa_both_nz:
        ...
        mov  ah, [FLT_B+0]
        cmp  al, ah
        jnb  fa_a_larger        ; BUGFIX: was jge (signed) - jnb = unsigned >= for biased exponents
...
; Inputs  : FLT_A, FLT_B
; Outputs : FLT_A = product
; Clobbers: AX, BX, CX, DX, DI, FLT_SA, FLT_ER
; =============================================================================
        mov  al, [FLT_A+0]
        or   al, al
        jnz  fmul_anz
...
fms_norm_ok:
        ...
        mov  bh, [FLT_ER]
        pop  si                 ; restore SI before tail-call to norm_pack
        jmp  norm_pack
...
fp_dig_done:
        ; Round 7-digit buffer to 6 significant digits using digit[6] as rounding guide.
        ; If digit[6] >= '5': round up digit[5] with carry, then blank digit[6].
        ; If digit[6] < '5':  digit[6] is kept as a genuine significant digit.
        mov  al, [IBUF+6]
        cmp  al, '5'
        jb   fp_strip           ; digit[6] < '5': skip rounding, keep as-is
        ; Round up: digit[6] >= '5'
        mov  bx, IBUF+5
fp_round_lp:
        inc  byte [bx]
        cmp  byte [bx], '9'+1
        jb   fp_blankseven      ; no overflow, done
        mov  byte [bx], '0'
        dec  bx
        cmp  bx, IBUF-1
        jg   fp_round_lp
        ; Carry out of all digits: 9999999 -> 10000000, bump de and set digit[0]='1'
        mov  byte [IBUF], '1'
        pop  ax                 ; de from stack
        inc  ax                 ; de++
        push ax
fp_blankseven:
        mov  byte [IBUF+6], '0' ; blank 7th slot: it was only a rounding guide
...
fpar_notdot:
        ...
        inc  si
        mov  [FLT_SP], si       ; save string pointer to dedicated 2-byte slot
        ...
        mov  si, [FLT_SP]       ; restore string pointer from dedicated 2-byte slot
