...
;         flt_print decimal point, flt_cmp byte-; =============================================================================
; MiniBASIC8088  MBF5 Float Library  v0.4-wip
; Copyright (c) 2026 Vincent Crabtree, MIT License
;
; 5-byte Microsoft Binary Format floating-point routines for 8088/8086.
...
; v0.4 : Precision improvements - COMPLETE.
;         flt_div: two-stage 32-bit quotient, full 7 sig fig precision.
;         flt_mul: step5 (A_byte4 * B_hi) >>8 added to CX for full 32-bit product.
;         BUGFIX: flt_mul step5 was doing 16-bit mul with dirty AH - added xor ah,ah.
;                 This was polluting flt_parse results with garbage bits.
;         BUGFIX: flt_add exponent compare used jge (signed) - should be jnb (unsigned).
;                 Biased exponents span 0x00..0xFF, signed compare breaks near 0x80.
;         BUGFIX: print_hex_byte used jmp output (tail call) causing stack leak on
;                 second nibble. Fixed to call output / ret.
;         Results: T1-T9 all correct. T3=-41.99999 T10=65533.99 (flt_mul precision limit
;                  with 3-byte partial products - see OPEN ITEMS below).
;
; ---------------------------------------------------------------------------
...
reset:  db   0xEA               ; far JMP to start
        ...
        times 4096-($-start) db 0xFForder, CLD guards.
; v0.3 : Fixed flt_mul exponent (DEC not INC FLT_ER when product<0.5),
;         flt_parse SI clobber (RAM save via FLT_TS), AH garbage (xor ah,ah),
;         flt_print FLT_T conflict (stack save), T5 test harness ordering,
;         flt_parse fractional part SI lost across flt_from_int_b calls.
